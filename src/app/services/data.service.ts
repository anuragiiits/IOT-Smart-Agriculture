...
export class DataService {
	...
	postItem(body){
		return this.http.post('http://127.0.0.1:8000/items/',body).map(res=>res.json());
	}
	postTranscation(body){
		return this.http.post('http://127.0.0.1:8000/transaction/',body).map(res=>res.json());
	}
	getBuyer(item_id)
	{
		return this.http.get('http://127.0.0.1:8000/transaction/?item_id='+item_id).map(res=>res.json());
	}
	// getTransactions(){
	// 	return this.http.get('http://127.0.0.1:8000/items/?username=garvit').map(res=>res.json());
	// }
